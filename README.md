...
Movie catalog management for the Neo4flix platform — CRUD operations, search and filtering, and JWT-secured write access.
...
```bash
git clone https://github.com/johneliud/movie-service.git
cd movie-service
```
...
Ensure a Neo4j instance is running and accessible at the configured URI. No schema migrations are required — Spring Data Neo4j creates the node constraints automatically on startup.
...
- [Architecture](docs/architecture.md) — system position, package structure, security model, Neo4j data model
